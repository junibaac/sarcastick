...
let REPO_OWNER = "ConstantinBicajanu"; 
let REPO_NAME = "memes";
let OCTOKIT = null;
...
uploadSubmitBtn.addEventListener('click', async () => {
    if (!OCTOKIT) { alert("GitHub non connesso!"); return; }
    if (fileUploader.files.length === 0) { alert("Scegli un file!"); return; }
...
function renderGallery(memes, containerId = 'gallery-container') {
    ...
    if (memes.length === 0) {
        container.innerHTML = '<p style="color:#666; width:100%; text-align:center;">Il Reticolo è vuoto. Carica il primo meme!</p>';
        return;
    }
...
function createPostCard(post) {
    ...
    card.onclick = (e) => { if(e.target.tagName !== 'SPAN') openModal(post); };
...
function renderHallOfFame() {
    const sorted = [...MEME_DATABASE].sort((a,b) => b.likedBy.length - a.likedBy.length);
...
function updateModalLikeUI() {
    ...
    const likeBtn = document.querySelector('.action-btn[onclick="voteCurrent()"]');
    if (likeBtn) {
...
function renderComments(comments) {
    ...
    if (comments.length === 0) {
        commentList.innerHTML = '<div style="color:#90949c; text-align:center; padding:10px;">Vuoto spaziale...</div>';
        return;
    }
    comments.forEach(c => {
